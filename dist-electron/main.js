...
import require$$3 from "oracledb";
...
class Builder extends EventEmitter$3 {
  ...
  join(table, first2, ...args) {
    ...
    if (typeof first2 === "function") {
      join2 = new JoinClause$1(table, joinType, schema);
      first2.call(join2, join2);
    } else if (joinType === "raw") {
      join2 = new JoinClause$1(this.client.raw(table, first2), "raw");
    } else {
      join2 = new JoinClause$1(table, joinType, schema);
      if (first2) {
        join2.on(first2, ...args);
      }
    }
    this._statements.push(join2);
    return this;
  }
  using(tables) {
    throw new Error(
...
let QueryCompiler$1 = class QueryCompiler {
  ...
  _joinTable(join2) {
    return join2.schema && !(join2.table instanceof Raw$2) ? `${join2.schema}.${join2.table}` : join2.table;
  }
  ...
  join() {
    ...
    while (++i < joins.length) {
      ...
      if (join2.joinType === "raw") {
        ...
        sql += join2.joinType + " join " + wrap_$1(
          table,
          void 0,
          this.builder,
          this.client,
...
const Transaction2 = transaction$5;
const {
  executeQuery,
  enrichQueryObject
} = queryExecutioner;
...
let Client$2 = class Client extends EventEmitter {
  constructor(config = {}) {
    super();
    this.config = config;
    this.logger = new Logger2(config);
...
function parse$1(str) {
  ...
  const fs2 = config.sslcert || config.sslkey || config.sslrootcert ? require$$0$3 : null;
  if (config.sslcert) {
...
function requireSqliteTransaction() {
  ...
  const Transaction3 = transaction$5;
...
function requireBindings() {
  ...
  (function(module, exports) {
    var fs2 = require$$0$3, path2 = require$$0$4, fileURLToPath = requireFileUriToPath(), join2 = path2.join, dirname = path2.dirname, exists = fs2.accessSync && function(path22) {
      try {
    ...
    del() {
      ...
      if (Array.isArray(joins)) {
        for (const join2 of joins) {
          ...
          const joinWheres = [];
          for (const clause of join2.clauses) {
            joinWheres.push(
              this.whereBasic({
...
function requireMssqlQuerycompiler() {
  ...
  class QueryCompiler_MSSQL extends QueryCompiler3 {
    ...
    updateWithTriggers() {
      ...
      const join2 = this.join();
      const where = this.where();
      const order = this.order();
      const { returning } = this.single;
      const declaredTemp = this._buildTempTable(returning);
      return {
        sql: withSQL + declaredTemp + `update ${top ? top + " " : ""}${this.tableName} set ` + updates.join(", ") + (returning ? ` ${this._returning("update", returning, true)}` : "") + (join2 ? ` from ${this.tableName} ${join2}` : "") + (where ? ` ${where}` : "") + (order ? ` ${order}` : "") + (!returning ? this._returning("rowcount", "@@rowcount") : this._buildReturningSelect(returning)),
    ...
    standardUpdate() {
      ...
      const { returning } = this.single;
      return {
        sql: withSQL + `update ${top ? top + " " : ""}${this.tableName} set ` + updates.join(", ") + (returning ? ` ${this._returning("update", returning)}` : "") + (join2 ? ` from ${this.tableName} ${join2}` : "") + (where ? ` ${where}` : "") + (order ? ` ${order}` : "") + (!returning ? this._returning("rowcount", "@@rowcount") : ""),
        returning: returning || "@@rowcount"
      };
    }
    //#endregion
...
function requireMysqlQuerycompiler() {
  ...
  class QueryCompiler_MySQL extends QueryCompiler3 {
    ...
    update() {
      ...
      return (comments === "" ? "" : comments + " ") + withSQL + `update ${this.tableName}` + (join2 ? ` ${join2}` : "") + " set " + updates.join(", ") + (where ? ` ${where}` : "") + (order ? ` ${order}` : "") + (limit ? ` ${limit}` : "");
    }
    forUpdate() {
      return "for update";
    }
...
async function createWindow() {
  win = new BrowserWindow({
    ...
    width: 1200,
    // Aumentei a largura para acomodar melhor a interface
    height: 800,
    // E a altura
    webPreferences: {
      preload,
